...
class DiscrepancyMonitor:
    def __init__(self):
        ...
        # Consecutive-scan requirement (soft): flag triggers once confidence is
        # high AND this many net-positive detections have accumulated. A failing
        # scan DECREMENTS the count by 1 (see _update_confidence) instead of
        # zeroing it, so an occasional noisy/centroid-jumped scan no longer wipes
        # the streak - that hard reset is what kept the flag from latching at 20 Hz.
        self.min_consecutive_scans = rospy.get_param('~min_consecutive_scans', 10)
        ...
        self.max_cluster_spread = rospy.get_param('~max_cluster_spread', 0.6)
        ...
        # External enable/disable gate (smach publishes False during abort)
        self.enabled = True
        ...
        rospy.Subscriber('/discrepancy_monitor/enable', Bool,
    ...
    # ------------------------------------------------------------------
    # External enable/disable gate
    # ------------------------------------------------------------------

    def enable_callback(self, msg):
        if self.enabled and not msg.data:
            rospy.loginfo("discrepancy_monitor: DISABLED by state machine (abort sequence)")
            self.confidence = 0.0
            self.consecutive_count = 0
            self.prev_centroid = None
            self.flag_active = False
            self.flag_pub.publish(Bool(data=False))
        elif not self.enabled and msg.data:
            rospy.loginfo("discrepancy_monitor: RE-ENABLED by state machine")
    ...
    def _update_confidence(self, valid_hits):
        if valid_hits > self.points_threshold:
            ...
            # Soft reset: decrement instead of zeroing, so one bad scan does not
            # wipe an otherwise-sustained detection streak.
            self.consecutive_count = max(0, self.consecutive_count - 1)
            self.confidence -= self.decay_rate

        self.confidence = max(0.0, min(1.0, self.confidence))

        # Trigger on the confidence integrator alone. It already encodes temporal
        # persistence (cannot cross trigger_threshold without several sustained
        # compact+stable detections, decays only slowly) and is robust to the
        # intermittent location_stable failures that stall consecutive_count.
        # consecutive_count is retained below for logging/diagnostics only.
